...
;		6 - UART REGISTER ADDRESS
;		8 - MONITOR: ?M VS. !M SWITCH
;	       10 - MONITOR: MEMORY POINTER
DELAY	.EQ    12 ; PC FOR DELAY SUBROUTINE
HEXX	.EQ    13 ; MONITOR: HEX ADDRESS ACCUMULATOR
BAUD	.EQ    14 ; RE.1=BAUD RATE CONSTANT
;		    RE.0=USED FOR READ, TYPE
ASCII	.EQ    15 ; RF.1=ASCII I/O CHARACTER
;		    RF.0=USED FOR READ, TYPE
;
; .EQATES
;
TYPA	.EQ	0D3H		; TYPE CHARACTER MACRO
;
; UART
;
...
;
	.CR	1802
;	.OR	8000H		;HRJ for Membership Card
	.OR	0000H		;HRJ for Membership Card
...
SAVER	GLO	RF		; SAVE REGISTERS R0-RF IN RAM
	...
	SAV			; PUSH T INTO RAM
	DEC	R4
	LDI	#IFINT	; IF CALLED VIA INTERRUPT, RETURN
	PLO	R1
	...
	STR	R4
;
CONTINIT LDI	#CHANGE	; COPY CODE THAT CHANGES TO THE DESIRED PC
...
;	SEP	R3		; SO IT WILL RETURN TO "ENTRY"
	LDI	0E0H		; R6 <- UART REGISTER ADDRESS $E000
	PHI	R6
	LDI	0
	PLO	R6
	SEP	R5		; Straight to Monitor
;
...
NORAM	GHI	R0		; NO RAM: CAN'T SAVE REGISTERS
	ADI	1
	PHI	R3
	LDI	#ENTRY	; CHANGE PROGRAM COUNTER TO R3
	PLO	R3
...
;	.DB	080H
;
;	UART INPUT CODE HERE
;
	INC	R6		; SET ADDRESS OF UART STATUS REG IN R6 ($E001)
KEY1:	LDN	R6		; GET D FROM UART STATUS BYTE
	SHR     		; 
	BNF	KEY1		; WAIT 'TILL DATA AVAIL.

	DEC	R6		; SET ADDRESS OF UART DATA REG IN R6 ($E000)
	LDN	R6		; GET DATA
	STR	R6		; ECHO BACK
	PHI	ASCII
	BR	BZ
...
; NO DELAY ANYMORE, THE CODE CLOW DRIVES THE UART.
;
BEGIN2	INC	R6	; SET ADDRESS OF UART STATUS REG IN R6 ($E001)
BEGIN3	LDN	R6	; Check UART ready.
	SHR
	SHR
	BNF	BEGIN3
;
OUTPUT	GLO	RD
        DEC	R6	; SET ADDRESS OF UART DATA REG IN R6 ($E000)
        STR	R6	; Send character from D.
        GLO	ASCII
        SMI	00BH
        PLO	ASCII
        BR	NXCHAR
;
	.NO RESET+$01DD		; CONTINUATION OF ORIGINAL CODE
...
; SYNTAX ERROR
;
SYNERR	LDI	#TYPE5D	; POINT TO TYPE5D
	PLO	R3
	SEP	R3		; TYPE <CR>
	.DB	00DH
	SEP	R3		; <LF>
...
;
	LDI	#INTERUPT	; SET R1 FOR BREAKPOINT INTERRUPT
	PLO	R1
	GHI	R5
	SMI	2
	PHI	R1
	...
	.NO	RESET+02F4H		; PAGE JUMPS: ALLOW RELOCATABLE BRANCH TO PAGE 3
	BR	SYNERR
	NOP
	NOP
	BR	RESTART
...
;
;	.OR	RESET+0XXB8H	;HRJ offset
;	LDI	(R2.0)		; (R2.0)=>R2.0
;	PLO	R2
;	LDI	(D)		; (D)=>D
;	LBR	(R(P))		; GO TO USER PROGRAM
	.NO	RESET+03F0H		; PAGE JUMPS: ALLOWS RELOCATABLE LONG BRANCH
;				; TO PAGE 2.
;
